...
/* SRW */
double direct_BAP2(charge *, charge *);//Bap change
...
int main(int argc, char **argv) {
	...
	double tol; //= 1e-8;//BAP change from -8
	...
	ssystem *sys;
	double tolle;//bap change

	fprintf(stdout, "Error of less than 1e-6 guaranteed for ratio up to 1:100 for shortest to longest hexahedral dimension\n");
	int neighbor = 3;

	while (neighbor != 1 && neighbor != 2)
	{
		fprintf(stdout, "ACA interaction criteria:\n Type, 1 for nearest neighbours and 2 for second-nearest neighbours\n");
		scanf("%d", &neighbor);
		if (neighbor == 1)
		{
			fprintf(stdout, "nearest neighbours selected\n");
		}
		else if (neighbor == 2)
		{
			fprintf(stdout, "second-nearest neighbours selected\n");
		}
		else
		{
			fprintf(stdout, "Invalid choice.....try again\n");
		}
	}

	fprintf(stdout, "ACA Tolerance :\n");
	scanf("%lf", &tolle);
	fprintf(stdout, "ACA Tolerance is %e\n", tolle);

	fprintf(stdout, "GMRES Tolerance :\n");
	scanf("%lf", &tol);
	fprintf(stdout, "GMRES Tolerance is %e\n", tol);
...
#ifdef SU0914
	if (indsys->opts->show_version == ON) {
		printf("Version info is: %s\n", FHVERSION);
		printf("Written to '%s'.\n", indsys->opts->vfname);
		vinfo = fopen(indsys->opts->vfname, "w");
		if (!vinfo) {
			printf("Error opening file '%s' for write access.",
				indsys->opts->vfname);
			return 1;
		}
		fprintf(vinfo, "%s\n", FHVERSION);
	...
	opts = indsys->opts;
	//opts->mat_vect_prod = DIRECT; //Bap line

	opts->tol = tol;//bap comment out
	...
	Set_up_MLACA(sys, indsys->num_fils);//BAP change
	if (neighbor == 1)
	{
		top_level(sys, 2);
	}
	...
	{
		FILAMENT *ff = sys->fill[i]->fil;

		if (ff->length > ff->width && ff->length > ff->height)
		{
			ff->max_dim = ff->length;
		}
		else if (ff->width > ff->height)
		{
			ff->max_dim = ff->width;
		}
		else
		{
			ff->max_dim = ff->height;
		}


		if (ff->length < ff->width && ff->length < ff->height)
		{
			ff->min_dim = ff->length;
		}
		else if (ff->width < ff->height)
		{
			ff->min_dim = ff->width;
		...
		if ((ff->x[0] + ff->width*ww[0] + ff->height*hh[0]) > (ff->x[0] - ff->width*ww[0] + ff->height*hh[0]))
		{
			temp_x_max = (ff->x[0] + ff->width*ww[0] + ff->height*hh[0]);
			temp_x_min = (ff->x[0] - ff->width*ww[0] + ff->height*hh[0]);
		}
		else
		{
			temp_x_min = (ff->x[0] + ff->width*ww[0] + ff->height*hh[0]);
		...
		if ((ff->x[0] + ff->width*ww[0] - ff->height*hh[0]) > (ff->x[0] - ff->width*ww[0] - ff->height*hh[0]))
		{
			ff->x_max = (ff->x[0] + ff->width*ww[0] - ff->height*hh[0]);
			ff->x_min = (ff->x[0] - ff->width*ww[0] - ff->height*hh[0]);
		}
		else
		{
			ff->x_min = (ff->x[0] + ff->width*ww[0] - ff->height*hh[0]);
		...
		if (temp_x_max > ff->x_max)
		{
			ff->x_max = temp_x_max;
		}


		if (temp_x_min < ff->x_min)
		{
			ff->x_min = temp_x_min;
		}
		...
		if ((ff->x[1] + ff->width*ww[0] + ff->height*hh[0]) >(ff->x[1] - ff->width*ww[0] + ff->height*hh[0]))
		{
			temp_x_max = (ff->x[1] + ff->width*ww[0] + ff->height*hh[0]);
			temp_x_min = (ff->x[1] - ff->width*ww[0] + ff->height*hh[0]);
		}
		else
		{
			temp_x_min = (ff->x[1] + ff->width*ww[0] + ff->height*hh[0]);
		...
		if ((ff->y[0] + ff->width*ww[1] + ff->height*hh[1]) > (ff->y[0] - ff->width*ww[1] + ff->height*hh[1]))
		{
			temp_y_max = (ff->y[0] + ff->width*ww[1] + ff->height*hh[1]);
			temp_y_min = (ff->y[0] - ff->width*ww[1] + ff->height*hh[1]);
		}
		else
		{
			temp_y_min = (ff->y[0] + ff->width*ww[1] + ff->height*hh[1]);
		...
		if ((ff->y[0] + ff->width*ww[1] - ff->height*hh[1]) > (ff->y[0] - ff->width*ww[1] - ff->height*hh[1]))
		{
			ff->y_max = (ff->y[0] + ff->width*ww[1] - ff->height*hh[1]);
			ff->y_min = (ff->y[0] - ff->width*ww[1] - ff->height*hh[1]);
		}
		else
		{
			ff->y_min = (ff->y[0] + ff->width*ww[1] - ff->height*hh[1]);
		...
		if (ff->y_max < temp_y_max)
		{
			ff->y_max = temp_y_max;
		}
		if (ff->y_min > temp_y_min)
		{
			ff->y_min = temp_y_min;
		}


		if ((ff->y[1] + ff->width*ww[1] - ff->height*hh[1]) > (ff->y[1] - ff->width*ww[1] - ff->height*hh[1]))
		{
			temp_y_max = (ff->y[1] + ff->width*ww[1] - ff->height*hh[1]);
			temp_y_min = (ff->y[1] - ff->width*ww[1] - ff->height*hh[1]);
		}
		else
		{
			temp_y_min = (ff->y[1] + ff->width*ww[1] - ff->height*hh[1]);
		...
		if ((ff->z[0] + ff->width*ww[2] + ff->height*hh[2]) > (ff->z[0] - ff->width*ww[2] + ff->height*hh[2]))
		{
			temp_z_min = (ff->z[0] + ff->width*ww[2] + ff->height*hh[2]);
			temp_z_max = (ff->z[0] - ff->width*ww[2] + ff->height*hh[2]);
		}
		else
		{
			temp_z_max = (ff->z[0] + ff->width*ww[2] + ff->height*hh[2]);
		...
		if ((ff->z[0] + ff->width*ww[2] - ff->height*hh[2]) > (ff->z[0] - ff->width*ww[2] - ff->height*hh[2]))
		{
			ff->z_max = (ff->z[0] + ff->width*ww[2] - ff->height*hh[2]);
			ff->z_min = (ff->z[0] - ff->width*ww[2] - ff->height*hh[2]);
		}
		else
		{
			ff->z_min = (ff->z[0] + ff->width*ww[2] - ff->height*hh[2]);
		...
		if (temp_z_max > ff->z_max)
		{
			ff->z_max = temp_z_max;
		}

		if (temp_z_min < ff->z_min)
		{
			ff->z_min = temp_z_min;
		}
		...
		if ((ff->z[1] + ff->width*ww[2] + ff->height*hh[2]) >(ff->z[1] - ff->width*ww[2] + ff->height*hh[2]))
		{
			temp_z_min = (ff->z[1] + ff->width*ww[2] + ff->height*hh[2]);
			temp_z_max = (ff->z[1] - ff->width*ww[2] + ff->height*hh[2]);
		}
		else
		{
			temp_z_max = (ff->z[1] + ff->width*ww[2] + ff->height*hh[2]);
	...
	//benn = fopen("MLACA_all.txt", "w+");


	//for (int tol = 0; tol < 1; tol++)
	//{




	//MLACA_cal(sys->depth, sys->number_level_groups, sys->group_interaction_number, sys->interactions, sys->kid_num, sys->child_index, sys->fill, sys, 2);//enough knowlage to reconstruct Z matrix
	//fprintf(benn, "  MLACA memory (tol = %d): %d\n", 1, sys->memory);
	//sys->memory = 0;

	//}
	//fclose(benn);
	//do_direct(sys->next_iteration, sys->number_next_iteration, sys->fill, sys->depth, sys->number_level_groups, sys->child_index, sys->kid_num, sys);


	//FILE * benn2;

	//benn2 = fopen("MLACA_all2.txt", "w+");


	//for (int toll = 0; toll < 4; toll++)
	//{




	MLACA_cal(sys->depth, sys->number_level_groups, sys->group_interaction_number, sys->interactions, sys->kid_num, sys->child_index, sys->fill, sys, tolle);//enough knowlage to reconstruct Z matrix
																																							 //fprintf(benn2, "  SVD memory (tol = %d):%lld\n\n", 1, sys->memory);
																																							 //fprintf(benn2, "  MLACA memory (tol = %d):%lld\n\n", 1, sys->memory_aa);
																																							 //sys->memory = 0;
	...
	sys->direct_inductance = (double *)malloc(number_number * sizeof(double));
	int tally = 0;
	int meme = 0;
	for (int i = 0; i < sys->group_num; i++)//number of groups on lowest level 
	{
		for (int j = 0; j < sys->kid_num[sys->depth - 1][i]; j++)//number of filaments in observation group 
		{
			...
			for (int k = 0; k < sys->kid_num[sys->depth - 1][i]; k++)//number of filaments in observation group 
			{
				sys->direct_inductance[tally] = direct_BAP2(sys->fill[sys->child_index[sys->depth - 1][i][j] - 1], sys->fill[sys->child_index[sys->depth - 1][i][k] - 1]);
				//sys->direct_inductance[tally] = calcp(sys->fill[sys->child_index[sys->depth - 1][i][j] - 1], sys->fill[sys->child_index[sys->depth - 1][i][k] - 1], NULL);
				++tally;
				++meme;
			}
		}
	...
	for (int i = 0; i < sys->group_num; i++)//number of groups on lowest level 
	{
		for (int j = 0; j < sys->number_next_iteration[i]; j++)//near interaction groups
		{
			for (int k = 0; k < sys->kid_num[sys->depth - 1][i]; k++)//number of filaments in observation group 
			{
				...
				for (int l = 0; l < sys->kid_num[sys->depth - 1][sys->next_iteration[i][j]]; l++)//number of filaments in source group
				{
					//sys->direct_inductance[tally] = calcp(sys->fill[sys->child_index[sys->depth-1][i][k]-1], sys->fill[sys->child_index[sys->depth-1][sys->next_iteration[i][j]][l]-1], NULL);
					sys->direct_inductance[tally] = direct_BAP2(sys->fill[sys->child_index[sys->depth - 1][i][k] - 1], sys->fill[sys->child_index[sys->depth - 1][sys->next_iteration[i][j]][l] - 1]);
					++tally;
					meme += 1;
				}
			}
	...
	dir *= 8 / 10 ^ 6;
	fprintf(stdout, "\n");
	fprintf(stdout, "Direct memory = %d MB\n", dir);

	int aca_svd_mem = sys->memory;
	aca_svd_mem *= 8 / 10 ^ 6;
	fprintf(stdout, "MLACA-SVD memory = %d MB\n", aca_svd_mem);
	...
	num_sub_extern = indsys->num_sub_extern = pick_subset(opts->portlist,
		indsys);
	...
	*/
	...
	indsys->FinalY = (CX **)MatrixAlloc(num_extern, num_sub_extern,
		sizeof(CX));
	...
	indsys->Precond = (PRE_ELEMENT **)MattAlloc(num_mesh,
		sizeof(PRE_ELEMENT *));
	if (1 == 1) { /* let's always dump the residual history */
		indsys->resids = MatrixAlloc(num_sub_extern, user_maxiters,
			sizeof(double));
	...
	*/
	...
	if (opts->orderROM > 0) {
		...
		for (ext = indsys->externals, j = 0; ext != NULL;
			ext = ext->next, j++) {
			int_list *elem;
		...
		if (indsys->opts->mat_vect_prod != MULTIPOLE && !indsys->dont_form_Z) {
			...
			actual_order = ArnoldiROM(B, C, (double **)NULL, (char**)MRMt,
				num_mesh, num_extern, num_extern, opts->orderROM,
		...
		if ((fROM = fopen(outfname, "w")) == NULL)
			fprintf(stderr, "No open fROM\n");
		/* now dump the reduced order model */
		dumpROMequiv_circuit(fROM, indsys->Ar, indsys->Br, indsys->Cr,
			indsys->Dr, actual_order * num_extern, num_extern, num_extern,
	...
	for (freq = fmin, m = 0;
		...
		m++, freq = (
			fmin != 0 ? pow(10.0, log10(fmin) + m * logofstep) : 0.0)) {
		printf("Frequency = %lg\n", freq);
		...
		for (int j = 0; j < indsys->num_fils; j++)
		{
		fprintf(matrix, "%e\t", sys->matrix[i][j]);
		}
		fprintf(matrix, "\n");
		}
		...
		*/
		...
		long double or = (sys->fill[sys->child_index[a + 1][b][i] - 1]->fil->lenvect[0] / sys->fill[sys->child_index[a + 1][b][i] - 1]->fil->length)*(sys->fill[sys->child_index[a + 1][sys->interactions[a][b][k]][j] - 1]->fil->lenvect[0] / sys->fill[sys->child_index[a + 1][sys->interactions[a][b][k]][j] - 1]->fil->length);
		...
		//Z[sys->child_index[a + 1][sys->interactions[a][b][k]][j] - 1][sys->child_index[a + 1][b][i] - 1] = sum* or ;
		//++nummm;
		...
		*/
			...
			if (opts->dumpMats & MZMt) {
				if (m == 0) {
					if (opts->kind & MATLAB) {
						...
						printf("Saving MZMt...\n");
						savecmplx2(fp2, "MZMt", indsys->MtZM, indsys->num_mesh,
							indsys->num_mesh);
						fclose(fp2);
					}
					if (opts->kind & TEXT) {
...
#ifdef SU0914
		if (opts->output_currents == ON) {
			sprintf(freq_str, ".freq %g\n", freq);
			dump_current_output(freq_str);
		}
#endif // SU0914
		...
		if (opts->soln_technique == ITERATIVE) {
			if (indsys->precond_type == LOC) {
				...
				if (m == 0) {
					...
					err = spOrderAndFactor(indsys->sparMatrix, NULL, 1e-3, 0.0,
						1);
#ifndef OTHER
#ifdef SRW0814
					etime = srw_seconds();
			...
			if (!dont_form_Z) {
				...
				if (indsys->opts->dumpMats & MZMt) {
					...
					if (spFileMatrix(indsys->sparMatrix, outfname, "MZMt", 0, 1,
						1) == 0)
			...
			fprintf(stderr,
				"Internal error: Unknown type of soln_technique: %d\n",
				opts->soln_technique);
			exit(1);
		}
		...
		for (ext = get_next_ext(indsys->externals), i = 0; ext != NULL; ext =
			get_next_ext(ext->next), i++) {
			...
			if (opts->soln_technique == ITERATIVE) {
				...
				if (opts->mat_vect_prod == MULTIPOLE)
					gmres(MtZM, b, x0, inner, SetupComputePsi, num_mesh,
						maxiters, tol, sys, chglist, 2 * PI * freq, R,
						indsys, i);
				else
					gmres(MtZM, b, x0, inner, directmatvec, num_mesh, maxiters,
						tol, sys, chglist, 2 * PI * freq, R, indsys, i);
			...
			if (opts->dumpMats & GRIDS)//Bap comment out
									   //if(true)//Bap line change
			...
			if (indsys->opts->debug == ON) {
				...
				savecmplx(fptemp, fname, indsys->FinalY, num_extern,
					num_sub_extern);
				fclose(fptemp);
			}
		...
		EXTERNAL *testt2;
		/*
		FILE *imag_Y_real_Y = fopen("imag_Y_real_Y.txt", "w");
		fprintf(imag_Y_real_Y, "[");
		for (extcol = get_next_ext(indsys->externals), i = 0; extcol != NULL; extcol =
			get_next_ext(extcol->next), i++) {
			...
			for (ext = indsys->externals; ext != NULL; ext = ext->next) {
				...
				if (testt != NULL)
				{
					if (indsys->FinalY[ext->Yindex][extcol->col_Yindex].imag < 0)
						fprintf(imag_Y_real_Y, "%.17f%.17fi,", indsys->FinalY[ext->Yindex][extcol->col_Yindex].real, indsys->FinalY[ext->Yindex][extcol->col_Yindex].imag);
					else
						fprintf(imag_Y_real_Y, "%.17f+%.17fi,", indsys->FinalY[ext->Yindex][extcol->col_Yindex].real, indsys->FinalY[ext->Yindex][extcol->col_Yindex].imag);
				}
				else
			...
			testt2 = get_next_ext(extcol->next);
			if (testt2 != NULL)
				fprintf(imag_Y_real_Y, ";");
		...
		*/
		...
		if (i != indsys->num_sub_extern) {
			fprintf(stderr,
				"Huh?  columns calculated = %d  and num extern = %d\n", i,
		...
		if (num_extern == num_sub_extern) {
			...
			fprintf(fp3, "ADMITTANCE matrix for frequency = %lg %d x %d\n ",
				freq, num_extern, num_sub_extern);
...
//Start BAP direct function




/*Used to calculate the inductance values of the directly computed section on the impedance matrix.*/
double direct_BAP2(charge * fil111, charge *fil222)
{

	//return analytic_BAP(pc->fil, uu->fil, 10);
	FILAMENT * fil11 = fil111->fil;
	FILAMENT * fil22 = fil222->fil;

	double dot_prod = fil11->lenvect[0] * fil22->lenvect[0] + fil11->lenvect[1] * fil22->lenvect[1] + fil11->lenvect[2] * fil22->lenvect[2];
	...
	if (fil11->filnumber == fil22->filnumber)
	{
		double inductance;
		...
		if (fil11->min_dim == fil11->length)
		{
			...
			{
				inductance = dot_prod*aanalitic(fil11, fil22, 7, 7, 7)* MUOVER4PI;
			}
			else if (fil11->min_dim == fil11->width)
			{
				if (fil11->min_dim * 10 > fil11->height)
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 7, 7, 12)* MUOVER4PI;
				}
				else
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 7, 7, 27)* MUOVER4PI;
				}
			}
			else if (fil11->min_dim == fil11->height)
			{
				...
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 7, 12, 7)* MUOVER4PI;
				}
				else
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 7, 27, 7)* MUOVER4PI;
				}
			}
			else if (fil11->min_dim * 10 > fil11->width)
			{
				if (fil11->min_dim * 10 > fil11->height)
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 7, 12, 12)* MUOVER4PI;
				}
				else
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 7, 12, 27)* MUOVER4PI;
				}
			}
			else
			{
				if (fil11->min_dim * 10 < fil11->height)
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 7, 27, 27)* MUOVER4PI;
				}
				else
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 7, 27, 12)* MUOVER4PI;
				}
			}
		...
		else if (fil11->min_dim == fil11->height)
		{
			if (fil11->min_dim == fil11->width)
			{
				...
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 12, 7, 7)* MUOVER4PI;
				}
				else
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 27, 7, 7)* MUOVER4PI;
				}
			}
			else if (fil11->min_dim * 10 > fil11->width)
			{
				if (fil11->min_dim * 10 > fil11->length)
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 12, 12, 7)* MUOVER4PI;
				}
				else
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 27, 12, 7)* MUOVER4PI;
				}
			}
			else
			{
				if (fil11->min_dim * 10 < fil11->length)
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 27, 27, 7)* MUOVER4PI;
				}
				else
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 12, 27, 7)* MUOVER4PI;
				}
			}
		}
		...
		else if (fil11->min_dim == fil11->width)
		{
			...
			if (fil11->min_dim * 10 > fil11->height)
			{
				if (fil11->min_dim * 10 > fil11->length)
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 12, 7, 12)* MUOVER4PI;
				}
				else
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 27, 7, 12)* MUOVER4PI;
				}
			}
			else
			{
				if (fil11->min_dim * 10 < fil11->length)
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 27, 7, 27)* MUOVER4PI;
				}
				else
				{
					inductance = dot_prod*aanalitic(fil11, fil22, 12, 7, 27)* MUOVER4PI;
				}
			}
		}

		if (fil11->segm->lambda != 0.0)
			inductance += fil11->segm->r2
			* fil11->length / fil11->area;
		return inductance;
	}
	...
	if (450 * length < dist)
	{
		return	dot_prod*mutual_quadrature(fil11, fil22, 1, 1, 1, 1, 1, 1)* MUOVER4PI;
	}
	else if (12 * length < dist)
	{
		return	dot_prod*mutual_quadrature(fil11, fil22, 2, 2, 2, 2, 2, 2)* MUOVER4PI;
	}
	if (2.5*length < dist)
	{
		if (fil11->max_dim == fil11->length)
		{
			if (fil11->max_dim == fil11->length)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 3, 4, 4, 3, 4, 4)* MUOVER4PI;
			}
			else if (fil11->max_dim == fil11->width)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 3, 4, 4, 4, 3, 4)* MUOVER4PI;
			}
			else if (fil11->max_dim == fil11->height)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 3, 4, 4, 4, 4, 3)* MUOVER4PI;
			}
			else
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 3, 4, 4, 4, 4, 4)* MUOVER4PI;
			}
		}
		else if (fil11->max_dim == fil11->width)
		{
			if (fil11->max_dim == fil11->length)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 4, 3, 4, 3, 4, 4)* MUOVER4PI;
			}
			else if (fil11->max_dim == fil11->width)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 4, 3, 4, 4, 3, 4)* MUOVER4PI;
			}
			else if (fil11->max_dim == fil11->height)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 4, 3, 4, 4, 4, 3)* MUOVER4PI;
			}
			else
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 4, 3, 4, 4, 4, 4)* MUOVER4PI;
			}
		}
		else if (fil11->max_dim == fil11->height)
		{
			if (fil11->max_dim == fil11->length)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 4, 4, 3, 3, 4, 4)* MUOVER4PI;
			}
			else if (fil11->max_dim == fil11->width)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 4, 4, 3, 4, 3, 4)* MUOVER4PI;
			}
			else if (fil11->max_dim == fil11->height)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 4, 4, 3, 4, 4, 3)* MUOVER4PI;
			}
			else
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 4, 4, 3, 4, 4, 4)* MUOVER4PI;
			}
		}
		else
		{
			return	dot_prod*mutual_quadrature(fil11, fil22, 4, 4, 4, 4, 4, 4)* MUOVER4PI;
		}
	...
	else if (1.5 * length < dist)
	{
		if (fil11->min_dim == fil11->length)
		{
			...
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 4, 5, 5, 4, 5, 5)* MUOVER4PI;
			}
			else if (fil22->min_dim == fil22->width)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 4, 5, 5, 5, 4, 5)* MUOVER4PI;
			}
			else if (fil22->min_dim == fil22->height)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 4, 5, 5, 5, 5, 4)* MUOVER4PI;
			}
			else
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 4, 5, 5, 5, 5, 5)* MUOVER4PI;
			}
		}
		else if (fil11->min_dim == fil11->width)
		{
			if (fil22->min_dim == fil22->length)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 5, 4, 5, 4, 5, 5)* MUOVER4PI;
			}
			else if (fil22->min_dim == fil22->width)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 5, 4, 5, 5, 4, 5)* MUOVER4PI;
			}
			else if (fil22->min_dim == fil22->height)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 5, 4, 5, 5, 5, 4)* MUOVER4PI;
			}
			else
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 5, 4, 5, 5, 5, 5)* MUOVER4PI;
			}
		}
		else if (fil11->min_dim == fil11->height)
		{
			if (fil22->min_dim == fil22->length)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 5, 5, 4, 4, 5, 5)* MUOVER4PI;
			}
			else if (fil22->min_dim == fil22->width)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 5, 5, 4, 5, 4, 5)* MUOVER4PI;
			}
			else if (fil22->min_dim == fil22->height)
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 5, 5, 4, 5, 5, 4)* MUOVER4PI;
			}
			else
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 5, 5, 4, 5, 5, 5)* MUOVER4PI;
			}
		}
		else
		{
			return	dot_prod*mutual_quadrature(fil11, fil22, 5, 5, 5, 5, 5, 5)* MUOVER4PI;
		}
	}
	/*
	else if (1.1 * length < dist)
	{
	if (fil11->min_dim == fil11->length)
	{
	if (fil11->min_dim == fil11->width && fil11->min_dim == fil11->height)
	{
	return dot_prod*aanalitic(fil11, fil22, 5, 5, 5)* MUOVER4PI;
	}
	else if (fil11->min_dim == fil11->width)
	{
	if (fil11->min_dim*10 > fil11->height)
	{
	return dot_prod*aanalitic(fil11, fil22, 5, 5, 8)* MUOVER4PI;
	}
	else
	{
	return dot_prod*aanalitic(fil11, fil22, 5, 5, 10)* MUOVER4PI;
	}
	}
	else if (fil11->min_dim == fil11->height)
	{
	if (fil11->min_dim * 10 > fil11->width)
	{
	return dot_prod*aanalitic(fil11, fil22, 5, 8, 5)* MUOVER4PI;
	}
	else
	{
	return dot_prod*aanalitic(fil11, fil22, 5, 10, 5)* MUOVER4PI;
	}
	}
	else if (fil11->min_dim*10 > fil11->width)
	{
	if (fil11->min_dim * 10 > fil11->height)
	{
	return dot_prod*aanalitic(fil11, fil22, 5, 8, 8)* MUOVER4PI;
	}
	else
	{
	return dot_prod*aanalitic(fil11, fil22, 5, 8, 10)* MUOVER4PI;
	}
	}
	else
	{
	if (fil11->min_dim * 10 < fil11->height)
	{
	return dot_prod*aanalitic(fil11, fil22, 5, 10, 10)* MUOVER4PI;
	}
	else
	{
	return dot_prod*aanalitic(fil11, fil22, 5, 10, 8)* MUOVER4PI;
	}
	}
	...
	if (fil11->min_dim * 10 > fil11->length)
	{
	return dot_prod*aanalitic(fil11, fil22, 8, 5, 5)* MUOVER4PI;
	}
	else
	{
	return dot_prod*aanalitic(fil11, fil22, 10, 5, 5)* MUOVER4PI;
	}
	}
	else if (fil11->min_dim * 10 > fil11->width)
	{
	if (fil11->min_dim * 10 > fil11->length)
	{
	return dot_prod*aanalitic(fil11, fil22, 8, 8, 5)* MUOVER4PI;
	}
	else
	{
	return dot_prod*aanalitic(fil11, fil22, 10, 8, 5)* MUOVER4PI;
	}
	}
	else
	{
	if (fil11->min_dim * 10 < fil11->length)
	{
	return dot_prod*aanalitic(fil11, fil22, 10, 10, 5)* MUOVER4PI;
	}
	else
	{
	return dot_prod*aanalitic(fil11, fil22, 8, 10, 5)* MUOVER4PI;
	}
	}
	}
	...
	if (fil11->min_dim * 10 > fil11->length)
	{
	return dot_prod*aanalitic(fil11, fil22, 8, 5, 8)* MUOVER4PI;
	}
	else
	{
	return dot_prod*aanalitic(fil11, fil22, 10, 5, 8)* MUOVER4PI;
	}
	}
	else
	{
	if (fil11->min_dim * 10 < fil11->length)
	{
	return dot_prod*aanalitic(fil11, fil22, 10, 5, 10)* MUOVER4PI;
	}
	else
	{
	return dot_prod*aanalitic(fil11, fil22, 8, 5, 10)* MUOVER4PI;
	}
	}
	}
	}
	*/
	else
	{
			...
			&& (fil11->z_min - 0.1*length) < fil22->z_max && (fil11->z_max + 0.1*length) >  fil22->z_min)
		{
			if (fil11->min_dim == fil11->length)
			{
				if (fil11->min_dim == fil11->width && fil11->min_dim == fil11->height)
				{
					return dot_prod*aanalitic(fil11, fil22, 7, 7, 7)* MUOVER4PI;
				}
				else if (fil11->min_dim == fil11->width)
				{
					if (fil11->min_dim * 10 > fil11->height)
					{
						return dot_prod*aanalitic(fil11, fil22, 7, 7, 13)* MUOVER4PI;
					}
					else
					{
						return dot_prod*aanalitic(fil11, fil22, 7, 7, 31)* MUOVER4PI;
					}
				}
				else if (fil11->min_dim == fil11->height)
				{
					if (fil11->min_dim * 10 > fil11->width)
					{
						return dot_prod*aanalitic(fil11, fil22, 7, 13, 7)* MUOVER4PI;
					}
					else
					{
						return dot_prod*aanalitic(fil11, fil22, 7, 31, 7)* MUOVER4PI;
					}
				}
				else if (fil11->min_dim * 10 > fil11->width)
				{
					if (fil11->min_dim * 10 > fil11->height)
					{
						return dot_prod*aanalitic(fil11, fil22, 7, 13, 13)* MUOVER4PI;
					}
					else
					{
						return dot_prod*aanalitic(fil11, fil22, 7, 13, 31)* MUOVER4PI;
					}
				}
				else
				{
					if (fil11->min_dim * 10 < fil11->height)
					{
						return dot_prod*aanalitic(fil11, fil22, 7, 31, 31)* MUOVER4PI;
					}
					else
					{
						return dot_prod*aanalitic(fil11, fil22, 7, 31, 13)* MUOVER4PI;
					}
				}
			...
			else if (fil11->min_dim == fil11->height)
			{
				if (fil11->min_dim == fil11->width)
				{
					if (fil11->min_dim * 10 > fil11->length)
					{
						return dot_prod*aanalitic(fil11, fil22, 13, 7, 7)* MUOVER4PI;
					}
					else
					{
						return dot_prod*aanalitic(fil11, fil22, 31, 7, 7)* MUOVER4PI;
					}
				}
				else if (fil11->min_dim * 10 > fil11->width)
				{
					if (fil11->min_dim * 10 > fil11->length)
					{
						return dot_prod*aanalitic(fil11, fil22, 13, 13, 7)* MUOVER4PI;
					}
					else
					{
						return dot_prod*aanalitic(fil11, fil22, 31, 13, 7)* MUOVER4PI;
					}
				}
				else
				{
					if (fil11->min_dim * 10 < fil11->length)
					{
						return dot_prod*aanalitic(fil11, fil22, 31, 31, 7)* MUOVER4PI;
					}
					else
					{
						return dot_prod*aanalitic(fil11, fil22, 13, 31, 7)* MUOVER4PI;
					}
				}
			}
			...
			else if (fil11->min_dim == fil11->width)
			{
				...
				if (fil11->min_dim * 10 > fil11->height)
				{
					if (fil11->min_dim * 10 > fil11->length)
					{
						return dot_prod*aanalitic(fil11, fil22, 13, 7, 13)* MUOVER4PI;
					}
					else
					{
						return dot_prod*aanalitic(fil11, fil22, 31, 7, 13)* MUOVER4PI;
					}
				}
				else
				{
					if (fil11->min_dim * 10 < fil11->length)
					{
						return dot_prod*aanalitic(fil11, fil22, 31, 7, 31)* MUOVER4PI;
					}
					else
					{
						return dot_prod*aanalitic(fil11, fil22, 13, 7, 31)* MUOVER4PI;
					}
				}
			}
		}
		...
		else
		{
			if (fil11->min_dim == fil11->length)
			{
				if (fil22->min_dim == fil22->length)
				{
					return	dot_prod*mutual_quadrature(fil11, fil22, 4, 6, 6, 4, 6, 6)* MUOVER4PI;
				}
				else if (fil22->min_dim == fil22->width)
				{
					return	dot_prod*mutual_quadrature(fil11, fil22, 4, 6, 6, 6, 4, 6)* MUOVER4PI;
				}
				else if (fil22->min_dim == fil22->height)
				{
					return	dot_prod*mutual_quadrature(fil11, fil22, 4, 6, 6, 6, 6, 4)* MUOVER4PI;
				}
				else
				{
					return	dot_prod*mutual_quadrature(fil11, fil22, 4, 6, 6, 6, 6, 6)* MUOVER4PI;
				}
			}
			else if (fil11->min_dim == fil11->width)
			{
				if (fil22->min_dim == fil22->length)
				{
					return	dot_prod*mutual_quadrature(fil11, fil22, 6, 4, 6, 4, 6, 6)* MUOVER4PI;
				}
				else if (fil22->min_dim == fil22->width)
				{
					return	dot_prod*mutual_quadrature(fil11, fil22, 6, 4, 6, 6, 4, 6)* MUOVER4PI;
				}
				else if (fil22->min_dim == fil22->height)
				{
					return	dot_prod*mutual_quadrature(fil11, fil22, 6, 4, 6, 6, 6, 4)* MUOVER4PI;
				}
				else
				{
					return	dot_prod*mutual_quadrature(fil11, fil22, 6, 4, 6, 6, 6, 6)* MUOVER4PI;
				}
			}
			else if (fil11->min_dim == fil11->height)
			{
				if (fil22->min_dim == fil22->length)
				{
					return	dot_prod*mutual_quadrature(fil11, fil22, 6, 6, 4, 4, 6, 6)* MUOVER4PI;
				}
				else if (fil22->min_dim == fil22->width)
				{
					return	dot_prod*mutual_quadrature(fil11, fil22, 6, 6, 4, 6, 4, 6)* MUOVER4PI;
				}
				else if (fil22->min_dim == fil22->height)
				{
					return	dot_prod*mutual_quadrature(fil11, fil22, 6, 6, 4, 6, 6, 4)* MUOVER4PI;
				}
				else
				{
					return	dot_prod*mutual_quadrature(fil11, fil22, 6, 6, 4, 6, 6, 6)* MUOVER4PI;
				}
			}
			else
			{
				return	dot_prod*mutual_quadrature(fil11, fil22, 6, 6, 6, 6, 6, 6)* MUOVER4PI;
			}
		}
	...
	if (fil11->length > fil22->length && fil11->length > fil22->width && fil11->length > fil22->height && fil11->length > fil11->width && fil11->length > fil11->height)
	{
	length = fil11->length;
	}
	else if (fil22->length > fil22->width && fil22->length > fil22->height && fil22->length > fil11->width && fil22->length > fil11->height)
	{
	length = fil22->length;
	}
	else if (fil11->width > fil22->width && fil11->width > fil11->height && fil11->width > fil22->height)
	{
	length = fil11->width;
	}
	else if (fil11->height > fil22->width && fil11->height > fil22->height)
	{
	length = fil11->height;
	}
	else if (fil22->width > fil22->height)
	{
	length = fil22->width;
	}
	else
	...
	*/
	...
	if (length * 100 < dist)
	{
	return	dot_prod*Gaussian_BAP(fil11, fil22, 2)* MUOVER4PI;
	}
	else if (length * 25 < dist)
	{
	return	dot_prod*Gaussian_BAP(fil11, fil22, 3)* MUOVER4PI;
	}
	else if (length * 10 <  dist)
	{
	return dot_prod*Gaussian_BAP(fil11, fil22, 4)* MUOVER4PI;
	}
	else if (length * 6 <  dist)
	{
	return dot_prod*Gaussian_BAP(fil11, fil22, 5)* MUOVER4PI;
	}
	else if (length * 4 <  dist)
	{
	return dot_prod*Gaussian_BAP(fil11, fil22, 6)* MUOVER4PI;
	}
	else if (length * 1.7 <  dist)
	{
	return dot_prod*aanalitic(fil11, fil22, 10)* MUOVER4PI;
	}
	else if (length * 1.3 < dist)
	{
	return dot_prod*aanalitic(fil11, fil22, 10)* MUOVER4PI;
	}
	else if (length * 1.2 <  dist)
	{
	return dot_prod*aanalitic(fil11, fil22, 10)* MUOVER4PI;
	}
	else if (length * 1.1 <  dist)
	{
	return dot_prod*aanalitic(fil11, fil22, 10)* MUOVER4PI;
	}
	else
	...
	{
	return dot_prod*aanalitic(fil11, fil22, 15)* MUOVER4PI;
	}
	else if (length * 1.2 <  dist)
	{
	return dot_prod*aanalitic(fil11, fil22, 20)* MUOVER4PI;
	}
	else if (length * 1.1 <  dist)
	{
	return dot_prod*aanalitic(fil11, fil22, 25)* MUOVER4PI;
	}
	else
	{
	return dot_prod*aanalitic(fil11, fil22, 30)* MUOVER4PI;
	}
	//return 0;
	...
	*/
	...
	//}
	/*
	else if (shor * 5 <  length)
	{
	if (length * 50 < dist)
	{
	return	dot_prod*Gaussian_BAP(fil11, fil22, 1)* MUOVER4PI;
	}
	else if (length * 5 < dist)
	{
	return	dot_prod*Gaussian_BAP(fil11, fil22, 3)* MUOVER4PI;
	}
	else
	...
	*/
	...
	/*
	if (seg1->lambda != 0.0)
	Z[filnum_m][filnum_m] += seg1->r2
	* fil_j->length / fil_j->area;
	*/ //self term 
	return 0;
}
//End BAP direct function
	...
	for (i = 0; i < Hinc / 2 || (Hinc % 2 == 1 && i == Hinc / 2); i++) {
		...
		*/
		if (i == 0)
			...
			h_from_edge += min_height / 2 * pow(r_height, (double)(i - 1))
			* (1 + r_height);
		...
		for (j = 0; j < Winc / 2 || (Winc % 2 == 1 && j == Winc / 2); j++) {
			...
			*/
			...
			if (j == 0)
				...
				w_from_edge += min_width / 2 * pow(r_width, (double)(j - 1))
				* (1 + r_width);
...
void fillA(SYS *indsys) {
	...
	for (seg = indsys->segment; seg != NULL; seg = seg->next) {
		...
		for (i = 0; i < seg->num_fils; i++) {
			fil = &seg->filaments[i];
			Alist[node1->index] = insert_in_list(
				make_melement(fil->filnumber, fil, 1), Alist[node1->index]);
			Alist[node2->index] = insert_in_list(
				make_melement(fil->filnumber, fil, -1),
				Alist[node2->index]);
		}
	}
	...
	if (counter != indsys->num_real_nodes - indsys->num_trees + 1) {
		fprintf(stderr,
			"Internal error when forming A: counter %d != num_real_nodes %d\n",
			...
			if (indsys->opts->mat_vect_prod != MULTIPOLE
				...
				for (seg2 = indsys->segment; seg2 != NULL; seg2 = seg2->next) {
					for (m = 0; m < seg2->num_fils; m++) {
						...
						else if (filnum_m > filnum_j) /*we haven't done it yet */

							Z[filnum_m][filnum_j] = Z[filnum_j][filnum_m] =
							mutual(fil_j, fil_m);
					}
				}
...
void savemats(SYS *indsys) {
	...
	if (opts->dumpMats & DUMP_A) {
		...
		if (opts->kind & MATLAB) {
			...
			dump_M_to_matlab(fp, &Alist[1], num_real_nodes - 1, nnz - nnz0,
				"A");
			fclose(fp);
		}
	}
		...
		*/
		...
		if (!indsys->dont_form_Z && indsys->opts->mat_vect_prod == DIRECT) {
			...
			if (indsys->dont_form_Z)
				...
				printf(
					"L matrix not dumped.  Run with mat_vect_prod = DIRECT if this is desired\n");
		}
	}
	...
	if (opts->kind & MATLAB && opts->dumpMats & DUMP_RL) {
		...
		*/
		...
		savemat_mod(fp, machine_type() + 100, "R", 1, num_fils, 0, R,
			(double *)NULL, 0, num_fils);

		if (!indsys->dont_form_Z && indsys->opts->mat_vect_prod == DIRECT) {
			/* do imaginary part of Z */
			for (i = 0; i < num_fils; i++) {
				savemat_mod(fp, machine_type() + 100, "L", num_fils, num_fils,
					0, Z[i], (double *)NULL, i, num_fils);
			}
		}
		else {
			if (indsys->dont_form_Z)
...
void savecmplx(FILE *fp, char *name, CX **Z, int rows, int cols) {
	...
	for (i = 0; i < rows; i++)
		for (j = 0; j < cols; j++)
			savemat_mod(fp, machine_type() + 100, name, rows, cols, 1,
				&Z[i][j].real, (double *)NULL, i + j, 1);

	/* do imaginary part of Z */
	for (i = 0; i < rows; i++)
		for (j = 0; j < cols; j++)
			savemat_mod(fp, machine_type() + 100, name, rows, cols, 0,
				&Z[i][j].imag, (double *)NULL, 1, 1);
}
		...
		for (j = 0; j < nmesh; j++) {
			...
			for (melem = Mlist[mesh]; melem != NULL; melem = melem->mnext) {
				if (melem->filindex < nmesh)
					...
					tempsum += trows[melem->filindex - nmesh][j].real
					* melem->sign;
			}
	...
	for (i = 0; i < num_mesh; i++) {
		for (j = i; j < num_mesh; j++) {
			...
			for (melem = Mlist[i]; melem != NULL; melem = melem->mnext) {
				for (melem2 = Mlist[j]; melem2 != NULL; melem2 =
					melem2->mnext) {
					if (melem2->filindex == melem->filindex)
						tempsum += melem->sign * R[melem->filindex]
						* melem2->sign;
				}
			}
	...
	for (j = 0; j < meshes; j++) {
		...
		for (m = Mlist[j]; m != NULL; m = m->mnext) {
			...
			if (last_filindex == m->filindex)
				printf(
					"Internal Warning: Mesh %d contains the same filament multiple times\n",
					j);
...
void compare_meshes(MELEMENT *mesh1, MELEMENT *mesh2) {
	...
	while (mesh1 != NULL && mesh2 != NULL && mesh1->filindex == mesh2->filindex
		&& mesh1->sign == mesh2->sign) {
		mesh1 = mesh1->mnext;
		mesh2 = mesh2->mnext;
	}
...
void pick_ground_nodes(SYS *indsys) {
	...
	for (atree = indsys->trees; atree != NULL; atree = atree->next) {
		...
		if (type == NORMAL)
			node = getrealnode(
			((SEGMENT *)atree->loops->path->seg.segp)->node[0]);
		else
			node = getrealnode(
			((PSEUDO_SEG *)atree->loops->path->seg.segp)->node[0]);
