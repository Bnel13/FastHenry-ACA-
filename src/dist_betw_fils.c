 ...
 */
double dist_betw_pt_and_fil(FILAMENT *fil_line, double *D, double *s, double DD,
		FILAMENT *fil, double t) {
	...
	if (tnew <= 1 && tnew >= 0) {
		...
		return dist_between(e[XX], e[YY], e[ZZ], fil_line->x[idx],
				fil_line->y[idx], fil_line->z[idx]);
	}
}
