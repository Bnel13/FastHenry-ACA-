...
/* int order;       array is 2*order+1 x 2*order+1 */
{
	...
	for (i = 0; i <= 2 * order; i++) {
		...
		if (i > 0 && i < 2 * order)
			array[i + 1][i] = i + 1; /* do first sub diagonal */
	}
...
/* double x, y, z, xp, yp, zp;	multipole and local cube centers */
{
	...
	double **mat = NULL; /* the transformation matrix */
			...
			for (n = 0, rhoN = rho; n <= order; rhoN *= (-rho), n++) {
				for (m = 0; m <= n; m++) { /* loop on On^m's, multipole moments */
					...
					if (k == 0) { /* use abbreviated formulae in this case */
						...
						if (m != 0) {
							temp1 = tleg[CINDEX(j + n, m)]
									* facFrA[j + n - m][n + m];
							mat[CINDEX(j, 0)][CINDEX(n, m)] += temp1 * cosB(m)
									/ rhoFac;
							mat[CINDEX(j, 0)][SINDEX(n, m, ct)] += temp1
									* sinB(m) / rhoFac;
						} else
							mat[CINDEX(j, 0)][CINDEX(n, 0)] += tleg[CINDEX(
									j + n, 0)] * facFrA[j + n][n] / rhoFac;
					} else {
						temp1 = tleg[CINDEX(j + n, abs(m - k))]
								* facFrA[j + n - abs(m - k)][n + m]
								* iPwr(abs(k - m) - k - m);
						temp2 = tleg[CINDEX(j + n, m + k)]
								* facFrA[j + n - m - k][n + m];
						temp3 = tleg[CINDEX(j + n, k)] * facFrA[j + n - k][n]
								* 2;

						/* generate bar(N)j^k entry */
						if (m != 0) {
							mat[CINDEX(j, k)][CINDEX(n, m)] += (temp1
									* cosB(m - k) + temp2 * cosB(m + k))
									/ rhoFac;
							mat[CINDEX(j, k)][SINDEX(n, m, ct)] += (temp1
									* sinB(m - k) + temp2 * sinB(m + k))
									/ rhoFac;
						} else
							mat[CINDEX(j, k)][CINDEX(n, 0)] += temp3 * cosB(k)
									/ rhoFac;

						/* generate dblbar(N)j^k entry */
						if (m != 0) {
							mat[SINDEX(j, k, ct)][CINDEX(n, m)] += (-temp1
									* sinB(m - k) + temp2 * sinB(m + k))
									/ rhoFac;
							mat[SINDEX(j, k, ct)][SINDEX(n, m, ct)] += (temp1
									* cosB(m - k) - temp2 * cosB(m + k))
									/ rhoFac;
						} else
							mat[SINDEX(j, k, ct)][CINDEX(n, 0)] += temp3
									* sinB(k) / rhoFac;
					}
				}
			}
			...
			for (n = j, rhoN = rhoJ; n <= order; rhoN *= (-rho), n++) {
				for (m = 0; m <= n; m++) { /* loop on On^m's, old local moments */
					...
					if (k == 0 && n - j >= m) { /* use abbreviated formulae in this case */
						...
						/* generate only bar(N)j^0 entry (dblbar(N)j^0 = 0 always) */
						if (m != 0) {
							temp1 = tleg[CINDEX(n - j, m)]
									* facFrA[0][n - j + m] * rhoFac;
							mat[CINDEX(j, 0)][CINDEX(n, m)] += temp1 * cosB(m);
							mat[CINDEX(j, 0)][SINDEX(n, m, ct)] += temp1
									* sinB(m);
						} else
							mat[CINDEX(j, 0)][CINDEX(n, 0)] += tleg[CINDEX(
									n - j, 0)] * facFrA[0][n - j] * rhoFac;
					} else {
						if (n - j >= abs(m - k))
							temp1 = tleg[CINDEX(n - j, abs(m - k))]
									* facFrA[0][n - j + abs(m - k)]
									* iPwr(m - k - abs(m - k)) * rhoFac;
						if (n - j >= m + k)
							temp2 = tleg[CINDEX(n - j, m + k)]
									* facFrA[0][n - j + m + k] * iPwr(2 * k)
									* rhoFac;
						if (n - j >= k)
							temp3 = 2 * tleg[CINDEX(n - j, k)]
									* facFrA[0][n - j + k] * iPwr(2 * k)
									* rhoFac;
						...
						if (m != 0) {
							...
							if (n - j >= m + k) {
								mat[CINDEX(j, k)][CINDEX(n, m)] += temp2
										* cosB(m + k);
								mat[CINDEX(j, k)][SINDEX(n, m, ct)] += temp2
										* sinB(m + k);
							}
						} else if (n - j >= k)
							mat[CINDEX(j, k)][CINDEX(n, 0)] += temp3 * cosB(k);
						...
						if (m != 0) {
							...
							if (n - j >= m + k) {
								mat[SINDEX(j, k, ct)][CINDEX(n, m)] += (-temp2
										* sinB(m + k));
								mat[SINDEX(j, k, ct)][SINDEX(n, m, ct)] += temp2
										* cosB(m + k);
							}
						} else if (n - j >= k)
							mat[SINDEX(j, k, ct)][CINDEX(n, 0)] += temp3
	...
	for (m = 0; m <= order; m++) { /* lp on m in Mn^m */
		for (n = m; n <= order; n++) { /* loop over rows with same m */
			for (j = 0; j < numchgs; j++) { /* add factors to a row */
				if (m == 0)
					mat[CINDEX(n, m)][j] *= fact(n); /* j! part of bar(N)j^0 */
				else { /* for Nj^k, k != 0 */
