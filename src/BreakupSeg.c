		...
		if (seg->length > length / DIVFACT) {
			if (indsys->opts->auto_refine == ON || is_initial == TRUE) {
				if (seg->type == NORMAL)
					...
					fprintf(stdout,
							"  Segment length: %lf,  maximum allowed length: %lf",
							seg->length, length / DIVFACT);
					fprintf(stdout,
							"  The segment is probably part of a ground plane.\n");
					fprintf(stdout,
				...
				if (indsys->opts->mat_vect_prod == MULTIPOLE)
					fprintf(stdout,
							"Warning: couldn't refine segments as needed because auto_refine == OFF.\n This will decrease multipole accuracy.\n");
			}
...
void BreakupSeg(SEGMENT *seg, double maxlength, charge *charges, SYS *indsys) {
	...
	for (i = 1; i < pieces; i++) {
		...
		if (i != pieces - 1) {
			...
			newnode = makenode(name, indsys->num_nodes++, x, y, z,
					nodelast->type,
					NULL);
		...
		newseg = makeseg(name, nodelast, newnode, seg->height, seg->width,
				...
				seg->lambda,
#endif
				seg->hinc, seg->winc, seg->r_height, seg->r_width,
				seg->widthdir, indsys->num_segs++, NORMAL, NULL);
