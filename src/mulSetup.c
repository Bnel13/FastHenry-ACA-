...
/* int flag;			ON => set depth automatically */
{
	...
	for (maxTileLength = 0.0, nextq = charges; nextq != NULL;
			nextq = nextq->next) {
	...
	if (length0 < maxTileLength) {
		if (indsys->opts->debug == ON) {
			fprintf(stderr,
					"length of cube smaller than max fil length. %lg<%lg\n",
	...
	if (indsys->opts->init_refine > 0)
		DivideSegs(
				(1.01 * length0) / pow(2.0, (double) indsys->opts->init_refine),
				charges, indsys, TRUE);
	...
	fprintf(stdout, "Total number of filaments before refine: %d\n",
			totalq);
				...
				for (k = 0; k < side; k++) {
					for (l = 0; l < side; l++) {
						if (cubes[i][j][k][l] != NULL) {
							...
							cubes_this_level++;
							Precond_Cost += OneCubeCost(cubes, i, j, k, l, side,
									&Direct_Cost);
						}
					}
				}
	...
	fprintf(stdout, "Total number of filaments after refine: %d\n",
			totalq);
	...
	for (nextq = charges; nextq != NULL; nextq = nextq->next) {
		...
		for (compflag = FALSE, i = (nextc->upnumeles[0] - 1); i >= 0; i--) {
			compq = nextc->chgs[i];
			if ((compq->x == nextq->x) && (compq->y == nextq->y)
					&& (compq->z == nextq->z)) {
#if 1== 0
...
#endif
				fprintf(stderr,
						"placeq: Warning: filaments had identical centers.  This may correspond to a nonphysical overlap.\n");
				fprintf(stderr, "  Make sure they are perpendicular.\n");
				fprintf(stderr,
						"  center = (%g %g %g). Filaments are from seg = `%s' and seg `%s'",
						compq->x, compq->y, compq->z, compq->fil->segm->name,
						nextq->fil->segm->name);
			}
		}
				...
				for (l = 0; l < side; l++) {
					...
					if (nextc != NULL) {
						...
						if (i < sys->depth) {
							...
							nextc->kids[1] = cubes[i + 1][2 * j][2 * k][2 * l
									+ 1]; /* null pointers */
							nextc->kids[2] = cubes[i + 1][2 * j][2 * k + 1][2
									* l];
							nextc->kids[3] = cubes[i + 1][2 * j][2 * k + 1][2
									* l + 1];
							nextc->kids[4] = cubes[i + 1][2 * j + 1][2 * k][2
									* l];
							nextc->kids[5] = cubes[i + 1][2 * j + 1][2 * k][2
									* l + 1];
							nextc->kids[6] =
									cubes[i + 1][2 * j + 1][2 * k + 1][2 * l];
							nextc->kids[7] =
									cubes[i + 1][2 * j + 1][2 * k + 1][2 * l + 1];
						}
						if (i > 0) {
				...
				for (l = 0; l < side; l++) {
					...
					if (nextc != NULL) {
						...
						if (num_real_kids == 0) {
							...
							m = 0;
							while (m < nextc->parent->numkids
									&& nextc->parent->kids[m] != nextc)
								m++;
							if (nextc != nextc->parent->kids[m]) {
								fprintf(stderr,
										"Couldn't find kid in parent's list. Exiting\n");
								exit(1);
							}
							/* remove from parent's list */
							fprintf(stderr,
									"Removing abandoned cube[%d][%d][%d][%d]\n",
									i, j, k, l);
					...
					if (nc != NULL) {
						if (i == depth) {
							...
							for (m = 0; m < nc->numkids; m++) {
								...
								if (nkid != NULL) {
									nc->upnumvects += 1;
									if (nkid->mul_exact == FALSE)
										all_mul_exact = FALSE;
									else {
										num_eval_pnts += nkid->upnumeles[0];
										for (p = 0; p < nkid->upnumeles[0];
												p++) {
											if (!nkid->chgs[p]->dummy)
												numchgs++;
										}
									}
									if (nkid->loc_exact == FALSE)
										all_loc_exact = FALSE;
								}
							}
							...
							if ((i <= sys->real_depth)
								...
								CALLOC(nc->chgs, num_eval_pnts, charge*, ON,
										AMSC);
								num_eval_pnts = 0;
								for (m = 0, first = TRUE; m < nc->numkids;
										m++) {
									nkid = nc->kids[m];
									if (nkid != NULL) {
										if (first == TRUE) {
											...
											if (nc->nbr_is_dummy == NULL)
												CALLOC(nc->nbr_is_dummy, 1,
														int*, ON, AMSC);
											nc->nbr_is_dummy[0] =
													nkid->nbr_is_dummy[0];
											first = FALSE;
										}
										for (n = 0; n < nkid->upnumeles[0];
												n++) {
											nc->chgs[num_eval_pnts++] =
													nkid->chgs[n];
										}
									}
								}
							...
							if (i <= sys->real_depth
									&& (!all_loc_exact
											|| (num_eval_pnts > numterms))) {/* le req'd*/
								nc->loc_exact = FALSE;
								nc->localsize = multerms(sys->order);
							} else if (i > sys->real_depth
									|| (all_loc_exact
											&& (num_eval_pnts <= numterms))) {
								num_lexact++;
								nc->loc_exact = TRUE;
...
/* int depth;			number of lowest level */
{
	...
	if (cp->level == depth)
		return (cp->upnumeles[0]);
	else
		for (i = 0; i < cp->numkids; i++)
				...
				for (l = 0; l < side; l++) {
					...
					if (nc != NULL) {
						if (nc->mul_exact == TRUE) {
							num_chgs = 0;
							for (p = 0; p < nc->upnumeles[0]; p++) {
								if (!nc->nbr_is_dummy[0][p])
									num_chgs++;
							}
							mul_maxq = MAX(mul_maxq, num_chgs);
							if (i == depth)
								mul_maxlq = MAX(mul_maxlq, num_chgs);
						}
						if (nc->loc_exact == TRUE) {
							loc_maxq = MAX(loc_maxq, nc->upnumeles[0]);
							if (i == depth)
								loc_maxlq = MAX(loc_maxlq, nc->upnumeles[0]);
						}
					}
				}
			}
			...
			if (nc->level == depth) {
				...
				for (i = 0; i < nc->numkids && kids_are_exact; i++) {
					if (nc->kids[i] == NULL)
						continue;
					all_null = FALSE;
					if (!nc->kids[i]->mul_exact)
						kids_are_exact = FALSE;
					else { /* count real panels */
						for (l = 0; l < nc->kids[i]->upnumeles[0]; l++) {
							if (!((nc->kids[i]->nbr_is_dummy[0])[l]))
								real_panel_cnt++;
						}
					}
				}
			...
			if (nc->level == depth) {
				...
				for (i = 0; i < nc->numkids && kids_are_exact; i++) {
					if (nc->kids[i] == NULL)
						continue;
					all_null = FALSE;
					if (!nc->kids[i]->loc_exact)
						kids_are_exact = FALSE;
					else
						real_panel_cnt += nc->kids[i]->upnumeles[0];
				}
			}
			if (kids_are_exact && !all_null)
	...
	for (cp = sys->directlist; cp != NULL; cp = cp->dnext) {
		for (i = 0; i < cp->upnumeles[0]; i++) {
			if (!(cp->nbr_is_dummy[0][i] = cp->chgs[i]->dummy))
				cp->is_dielec[i] = (cp->chgs[i]->surf->type == DIELEC
