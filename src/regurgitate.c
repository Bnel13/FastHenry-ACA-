...
void spit(SYS *indsys, regurg_cb new_coords, char *suffix) {
...
#if SUPERCON == ON
			fprintf(stdout,
					"%s%s %s%s %s%s w=%lg h=%lg nhinc=%d nwinc=%d rw=%lg rh=%lg sigma=%lg lambda=%lg",
					seg->name, suffix, seg->node[0]->name, suffix,
					seg->node[1]->name, suffix, seg->width, seg->height,
					seg->hinc, seg->winc, seg->r_width, seg->r_height,
					seg->sigma, seg->lambda);
#else
	...
	for (gp = indsys->planes; gp != NULL; gp = gp->next) {
		...
		if (is_nonuni_gp(gp)) {
			fprintf(stdout,
					"Nonuniform planes not supported for regurgitation at this time!\n");
			continue;
		}
