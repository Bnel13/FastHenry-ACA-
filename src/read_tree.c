...
int process_plane(GROUNDPLANE *grndp, FILE *fp, SYS *indsys) {
	...
	for (contactp = gp->grndp->list_of_contacts; contactp != NULL; contactp =
			contactp->next) {
	...
	for (contactp = gp->grndp->list_of_contacts; contactp != NULL; contactp =
		...
		if (contactp->done == FALSE
				&& strncmp("equiv_rect", contactp->func, 10) != 0) {
			make_contacts(contactp, gp);
			contactp->done = TRUE;
		}
	...
	gp->grndp->numesh = gp->num_leaves
			+ (gp->num_z_pts - 1) * gp->num_seg_groups;
...
void get_nonuni_coords(double x, double y, double z, Nonuni_gp *gp, double *xr,
		double *yr, double *zr) {
	double rx, ry, rz;
...
void get_global_coords(double x, double y, double z, Nonuni_gp *gp, double *xg,
		double *yg, double *zg) {
	double xv, yv, zv;
...
void get_global_vec(double x, double y, double z, Nonuni_gp *gp, double *xg,
		double *yg, double *zg) {
...
int readTree(FILE *fp, Nonuni_gp *gp) {
	...
	if (sscanf(line, "%d", &num_cells) != 1) {
		fprintf(stderr,
				"First line should be an integer with total number of cells\n");
		return 1;
	}
	...
	for (i = 0; i < num_cells; i++) {
		...
		/* assume binary tree */
		numread = sscanf(line, "%d %s %s %d %d", &dum, typename, bintype,
				&nchild1, &nchild2);
		...
		if (strcmp(typename, "NONE") == 0) {
			...
			if (cells[dum - 1].children != NULL) {
				fprintf(stderr,
						"uh oh, this cell has been defined before: %d\n", dum);
			}

			if (cells[nchild1 - 1].parent != NULL)
				fprintf(stderr,
						"uh oh, child %d for cell %d already has a parent\n",
						nchild1, dum);
	...
	if (cells[0].parent != NULL) {
		fprintf(stderr,
				"uh oh, cells[0] has a parent but should be root cell!\n");
		return 1;
	}
...
void set_grid_coords(Grid_2d *grid, double x0, double y0, double x1, double y1) {
	...
	for (i = 0; i < grid->y_cells; i++)
		for (j = 0; j < grid->x_cells; j++) {
			x0n = x0 + j * xsize;
			y1n = y1 - i * ysize;
			set_cell_coords(grid->kids[i][j], x0n, y1n - ysize, x0n + xsize,
					y1n);
		}
}
...
void resolve_bi_children(Gcell *cell, Info *info) {
	...
	if (two_kids->type == NS) {
		...
		edges[NORTH] = make_two_edge(cell, two_kids->child1, two_kids->child2,
				NORTH);
		edges[SOUTH] = make_two_edge(cell, two_kids->child1, two_kids->child2,
				SOUTH);
...
G_nodes *make_new_node(double x, double y, int cell_dir, Gcell *cell, int index) {
	G_nodes *node;
...
void Combine_edges(Gcell *cell1, char dir1, Gcell *cell2, char dir2) {
	...
	if (c_is_leaf(cell1) || c_is_leaf(cell2))
		...
		if (edge1->num_children != edge2->num_children) {
			fprintf(stderr,
					"Bad discretization: number of children doesn't match %d != %d\n",
					edge1->num_children, edge2->num_children);
			exit(1);
		} else {
			/* recursively call for children */
			for (i = 0; i < edge1->num_children; i++)
				Combine_edges(edge1->children[i], dir1, edge2->children[i],
						dir2);
		}
	}
		...
		if (old_node->cells[i] != NULL) {
			if (new_node->cells[i] != NULL) {
				if (new_node->cells[i] != old_node->cells[i])
					GP_PANIC(
							"In replace_node: Can't combine conflicting cell info!");
				/* else do nothing */
			} else {
...
void determine_adjaceny(G_nodes *nodelist) {
	...
	for (node = nodelist; node != NULL; node = node->next) {
		...
		if (node->cells[NE] != node->cells[NW])
			node->adjacent[N] = get_adjacent_node(node, node->cells[NE], SW,
					WEST, node->cells[NW], SE, EAST);

		/* do east */
		if (node->cells[NE] != node->cells[SE])
			node->adjacent[E] = get_adjacent_node(node, node->cells[NE], SW,
					SOUTH, node->cells[SE], NW, NORTH);

		/* do south */
		if (node->cells[SE] != node->cells[SW])
			node->adjacent[S] = get_adjacent_node(node, node->cells[SE], NW,
					WEST, node->cells[SW], NE, EAST);
		/* do west */
		if (node->cells[NW] != node->cells[SW])
			node->adjacent[W] = get_adjacent_node(node, node->cells[NW], SE,
					SOUTH, node->cells[SW], NE, NORTH);
	}
}
...
void generate_segs(Nonuni_gp *gp, SYS *indsys) {
	...
	for (node = gp->nodelist; node != NULL; node = node->next) {
		...
		if (node->adjacent[N] != NULL) {
			...
			if (x_width != 0) {
				make_segs(N, node, node->adjacent[N], x_width, x_shift, 0.0, gp,
						indsys);
				gp->num_seg_groups++;
			}
		}
		if (node->adjacent[E] != NULL) {
			get_width_and_shift(Y_DIR, node, node->cells[SE], node->cells[NE],
					&y_width, &y_shift);
			if (y_width != 0) {
				make_segs(E, node, node->adjacent[E], y_width, 0.0, y_shift, gp,
						indsys);
				gp->num_seg_groups++;
			}
		}
...
void make_segs(char direction, G_nodes *node, G_nodes *othernode, double width,
	...
	for (i = 0; i < num_z_pts; i++) {
		/*
		 ...
		 gp->n_hinc, gp);
		 */
		segs[i] = make_one_seg(node->x + x_shift, node->y + y_shift, z_c[i],
				othernode->x + x_shift, othernode->y + y_shift, z_c[i], width,
				thick[i], wx, wy, wz, gp, indsys, &node0, &node1);
		node0->gp_node = node;
...
void draw_one_seg(char direction, double x1, double y1, double z1, double x2,
		double y2, double z2, double width, double height, double hx, double hy,
		double hz, double wx, double wy, double wz, int nhinc, Nonuni_gp *gp) {
...
void dump_cell(Gcell *cell, FILE *fp) {
	...
	fprintf(fp, "\nSelf: %d\t parent %d\t (x0 y0)(x1 y1) (%lg,%lg)(%lg,%lg)\n",
			cell->index, cell->parent->index, cell->x0, cell->y0, cell->x1,
			cell->y1);

	fprintf(fp, "  area %lg \tChildren type: %d\tChildren: ",
			(cell->y1 - cell->y0) * (cell->x1 - cell->x0),
			c_get_children_type(cell));
...
void dump_node(G_nodes *node, FILE *fp) {
	...
	fprintf(fp, "\nself: %d \tx,y (%lg,%lg)\t flag %d\tnesw adj: ", node->index,
			node->x, node->y, node->flag);
	for (i = 0; i < NUMADJ; i++)
...
void dump_nonuni_plane_currents(Nonuni_gp *gp, CX *Ib, FILE *fp) {
	...
	for (gnode = gp->nodelist; gnode != NULL; gnode = gnode->next) {
		...
		if (gnode->e_segs != NULL && gnode->n_segs != NULL)
			...
			fprintf(fp, "%lg %lg %lg   %lg +j %lg    %lg +j %lg\n", x, y, z,
					Ib[gnode->e_segs[0]->filaments[0].filnumber].real,
					Ib[gnode->e_segs[0]->filaments[0].filnumber].imag, 0.0,
					0.0);
		else if (gnode->n_segs != NULL)
			fprintf(fp, "%lg %lg %lg   %lg +j %lg    %lg +j %lg\n", x, y, z,
					0.0, 0.0, Ib[gnode->n_segs[0]->filaments[0].filnumber].real,
					Ib[gnode->n_segs[0]->filaments[0].filnumber].imag);
	}
}
