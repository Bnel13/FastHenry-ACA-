...
int checkplaneformula(double*, double*, double*, double, double, double, int,
		int, int);
double findsegmentwidth(double*, double*, double*, int, int, int, int);
void doincrement(double, double, double, double, double, double, int, double*,
		double*, double*);
void dounitvector(double, double, double, double, double, double, double*,
		double*, double*);
...
double lengthof(int p1, int p2, double *x, double *y, double *z) {
	...
	value = sqrt(
			((square((x[p2] - x[p1]))) + (square((y[p2] - y[p1])))
					+ (square((z[p2] - z[p1])))));
...
/* make up the vectors.                                                        */
double innerproduct(int p1, int p2, int p3, int p4, double *x, double *y,
		double *z) {
	double value;
...
/* make up the vectors.                                                      */
double findangle(int p1, int p2, int p3, int p4, double *x, double *y,
		double *z) {
...
/* given plane.                                                              */
int checkplaneformula(double *xcord, double *ycord, double *zcord, double x,
		double y, double z, int mid, int o1, int o2) {
	...
	if (fabs(value - equation)
			/ (xpart * xpart + ypart * ypart + zpart * zpart + 1e-200) < EPS) {
		return 1;
	} else {
		printf(
				"Possible internal error: something wrong with coordinates %lg %lg %lg\n",
				x, y, z);
		return 0;
	}
}
...
double findsegmentwidth(double *x, double *y, double *z, int mid, int o1,
		int o2, int dim) {
...
void findrefnodes(GROUNDPLANE *plane, NODES *begnode, NODES *endnode, int *b0,
		int *b1, int *e0, int *e1) {
	int node1, node2, i, j;
	NODES *tempnode;
...
SPATH *old_path_through_gp(NODES *nodein, NODES *nodeout, GROUNDPLANE *plane) {
	...
	tempnode = find_nearest_gpnode(nodein->x, nodein->y, nodein->z, plane, &bc0,
			&bc1);
	tempnode = find_nearest_gpnode(nodeout->x, nodeout->y, nodeout->z, plane,
			&ec0, &ec1);
		...
		for (j = 0; j < plane->seg1; j++) {
			...
			for (k = 0; k < 4; k++) {
				...
				for (n = 1; n < seg->num_fils; n++, tempmesh++) {
					melem = pMlist[tempmesh] = (MELEMENT *) MattAlloc(1,
							sizeof(MELEMENT));
...
#define MAXchoices 4
	...
	if (s1 > 0)
		nchoices += add_choice(&choices[nchoices], nodes_so_far,
				segs1[s1 - 1][s2], pnodes[s1 - 1][s2], s1 - s1_goal, -1, 0,
				s1_momentum, s2_momentum);
	...
	if (s2 > 0)
		nchoices += add_choice(&choices[nchoices], nodes_so_far,
				segs2[s1][s2 - 1], pnodes[s1][s2 - 1], s2 - s2_goal, 0, -1,
				s1_momentum, s2_momentum);
...
void dump_mesh_coords(SYS *indsys) {
	...
	for (i = 0; i < nmeshes && counter < buffer_rows - 4; i++) {
		for (melem = indsys->Mlist[i];
				melem != NULL && counter < buffer_rows - 4; melem =
						melem->mnext) {
			fil = melem->fil;
	...
	savemat_mod(fp, j + 100, "meshes", buffer_rows, buffer_cols, 0,
			line_list[0], (double *) NULL, 0, buffer_rows * buffer_cols);
	...
	for (i = 0; i < nmeshes; i++) {
		for (melem = indsys->Mlist[i];
			...
			fprintf(fp, "%lg %lg %lg %d\n %lg %lg %lg %d\n", fil->x[0],
					fil->y[0], fil->z[0], i + 1, fil->x[1], fil->y[1],
					fil->z[1], i + 1);
			counter += 2;
		}
