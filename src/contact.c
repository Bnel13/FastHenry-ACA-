...
void walk_along_line(double, double, double, double, double, double, double,
		double, Nonuni_gp*);
...
void limit_box(double, double, double, double, double, double, double, double,
...
void contact_equiv_rect(ContactList*, Nonuni_gp*, double, double, double,
		double);
void make_equiv_rect(double, double, double, double, double, Nonuni_gp*, char*);
void walk_along_edge(double, double, double, double, Nonuni_gp*, char, NODES*,
		char*, double);
void equiv_nodes_on_edge(G_nodes*, char, G_nodes*, NODES*, Nonuni_gp*, char*,
		double);
...
void contact_initial_mesh_grid(ContactList*, Nonuni_gp*, double, double, double,
		double);
...
ContactList *make_contactlist(ContactList *head, char *line, double units,
	...
	*skip = skip1 + skip2;
	...
	if (strcmp(name, "connection") == 0)
		...
		/* note: adds to skip */
		return make_contact_connection(head, line, units, relx, rely, relz,
				skip);
...
void contact_error2(char *errstr, char *line, char *nametype) {
	...
	fprintf(stderr, "%s\n%s", errstr, line);
	fprintf(stderr,
			"The contact format is 'contact <type> <nodename> (val1,val2,...)'\n");
	fprintf(stderr,
			"Where type is a string, and valn is a floating point value\n");
	exit(1);
}
...
void contact_line(ContactList *contactp, Nonuni_gp *gp, double relx,
	...
	if (!is_in_cell(x0, y0, contain))
		contact_error("First point of contact line not in plane!", "",
				contactp);

	contain = get_containing_cell(x1, y1, gp->root_cell);

	if (!is_in_cell(x1, y1, contain))
		contact_error("Second point of contact line not in plane!", "",
				contactp);
	...
	walk_along_line(x0, y0, z0, x1, y1, z1, vals[6] * units, vals[7] * units,
			gp);
...
double edge_coord(double x_final, double x_initial, double y_initial, double xv,
		double yv) {
...
void set_edge_nodes(G_nodes *node1, G_nodes *node2, char adj_dir, char dir1,
		char dir2, Gcell *cell) {
	G_nodes *node;

	for (node = node1->adjacent[adj_dir]; node != node2; node =
			node->adjacent[adj_dir]) {
		node->cells[dir1] = cell;
		node->cells[dir2] = cell;
	}
}

void find_or_make_node(Gcell *cell, Gcell *kid1, Gcell *kid2, char node_start,
		char node_end, double x, double y, Nonuni_gp *gp) {
	char adj_dir;
	G_nodes *node;
	...
	if (node != NULL) {
		...
		node = make_new_node(x, y, opposite_dir(node_start), kid1,
				++gp->num_nodes);
		...
		fix_adjacency(cell->bndry.nodes[node_start], adj_dir, node,
				cell->bndry.nodes[node_end]);
	}
}
...
void cut_inside_rect(double x, double y, double z, double rect_x_width,
		double rect_y_width, double max_cell_x, double max_cell_y,
		Nonuni_gp *gp) {
...
void contact_decay_rect(ContactList *contactp, Nonuni_gp *gp, double relx,
	...
	if (xl < gp->root_cell->x0) {
		contact_warning(
				"Left edge of decay rectangle not in plane.  Truncating.",
				contactp);
		xl = gp->root_cell->x0;
	}
	...
	if (xr > gp->root_cell->x1) {
		contact_warning(
				"Right edge of decay rectangle not in plane.  Truncating.",
				contactp);
		xr = gp->root_cell->x1;
	}
	...
	if (yl < gp->root_cell->y0) {
		contact_warning(
				"Bottom edge of decay rectangle not in plane.  Truncating.",
				contactp);
		yl = gp->root_cell->y0;
	}
	...
	if (yr > gp->root_cell->y1) {
		contact_warning(
				"Top edge of decay rectangle not in plane.  Truncating.",
				contactp);
		yr = gp->root_cell->y1;
	}
	...
	if (rect_x_width <= max_cell_x) {
		contact_warning(
				"rectangle x width <= max cell width. Will set max_cell = 0.99 * rect_width\nNote: Almost no gradual decay in x direction will result",
				contactp);
	...
	if (rect_y_width < max_cell_y) {
		contact_warning(
				"rectangle y width <= max cell width. Will set max_cell = 0.99 * rect_width\nNote: Almost no gradual decay in y direction will result",
				contactp);
...
void do_decay_rect(double x, double y, double z, double rect_x_width,
		double rect_y_width, double max_cell_x, double max_cell_y,
		double stop_x, double stop_y, Nonuni_gp *gp) {
	...
	limit_box(x, y, rect_x_width, rect_y_width, x_min, x_max, y_min, y_max, &xl,
			&yl, &xr, &yr);
	...
	while ((max_cell_x < stop_x || max_cell_y < stop_y || stop_x < 0
			|| stop_y < 0)
		...
		compute_new_widths(&rect_x_width, &rect_y_width, &max_cell_x,
				&max_cell_y);
...
void compute_new_widths(double *x_rect, double *y_rect, double *x_cell,
		double *y_cell)
...
/* char *name;  node name */
{
	...
	if (inside_nodes == NULL) {
		fprintf(stderr,
				"Error: No nodes inside equiv_rect %s. Rectangle must be made larger,\n     or plane must be refined more at that point\n",
				name);
		return;
	}
	...
	if (cnode != NULL)
		fprintf(stderr,
				"Warning: equiv_rect %s's center node appears to be part of another contact\n",
				name);
	else {
	...
	for (list = inside_nodes->next; list != NULL; list = list->next) {
		...
		if (node == NULL) {
			...
			sprintf(templine, "n%s_%s_pseudo_%lg_%lg", grndp->name, name,
					gnode->x, gnode->y);
			...
			if (getrealnode(node) != realcnode) {
				isalready_done = FALSE;
				fprintf(stderr,
						"Warning: contact appears to overlap another contact\n");
			} else
				isalready_done = TRUE;
...
void walk_along_edge(double xl, double yb, double xr, double yt, Nonuni_gp *gp,
		char which_edge, NODES *cnode, char *name, double z0)
...
/* *** this is no longer called *** */
void equiv_nodes_on_edge(G_nodes *node1, char dir, G_nodes *node2, NODES *cnode,
		Nonuni_gp *gp, char *name, double z0) {
...
NODES *make_fastH_node(G_nodes *node, GROUNDPLANE *grndp, Nonuni_gp *gp,
	...
	cnode = make_new_node_with_nonuni(node, pseudoname,
			grndp->indsys->num_nodes, xg, yg, zg, grndp->indsys, gp);

	/* make a pseudo_seg between other ground plane usernodes */
	/* (to be used by graph searching algs) */
	grndp->fake_seg_list = make_new_fake_segs(cnode, grndp->usernodes,
			grndp->fake_seg_list);
...
void contact_initial_grid(ContactList *contactp, Nonuni_gp *gp, double relx,
	...
	if (fabs(vals[0] - (int) vals[0]) > EPS
		...
		contact_error(
				"Both values should be integers: num cells in x, num in y.", "",
				contactp);
...
void make_initial_grid(Nonuni_gp *gp, int x_cells, int y_cells) {
	...
	if (c_get_children_type(root) != NONE) {
		fprintf(stderr,
				"Error: attempting to cut an initial grid that has already be divided.\n x_cells=%d, y_cells=%d\n",
				x_cells, y_cells);
		exit(1);
	}
...
/* calls make_new_node() to make a new node, then adds this one
 to the linked list */
G_nodes *add_new_node(double x, double y, int cell_dir, Gcell *cell, int index,
		Nonuni_gp *gp) {
	G_nodes *node;
...
ContactList *make_contact_connection(ContactList *head, char *line,
	...
	con_decay->func = (char *) Gmalloc(
			(strlen("decay_rect") + 1) * sizeof(char));
	con_equiv->func = (char *) Gmalloc(
			(strlen("equiv_rect") + 1) * sizeof(char));
	...
	if (*line != '(')
		contact_error2("Values for contact must start with '('", line,
				"connection");
	...
	if (*linep != ')')
		contact_error2("Contact values did not end with ')'", line,
				"connection");

	if (numvals != 6)
		contact_error2("connection type contact must have 6 values", line,
				"connection");
	...
	while (*linep != ')') {
		if (sscanf(linep, "%lf%n", &val, &skip4) != 1)
			contact_error2("Couldn't read value starting at:", linep,
					"connection");
		else {
			linep += skip4;
			if (i != 5) {
...
void contact_trace(ContactList *contactp, Nonuni_gp *gp, double relx,
	...
	if (!is_in_cell(x0, y0, contain))
		contact_error("First point of contact trace not in plane!", "",
				contactp);

	contain = get_containing_cell(x1, y1, gp->root_cell);

	if (!is_in_cell(x1, y1, contain))
		contact_error("Second point of contact trace not in plane!", "",
				contactp);

	if (vals[6] <= 0.0 || vals[7] < 1.0)
		contact_error(
				"Width of trace must be greater than zero, and scale factor greater than 1.",
				"", contactp);
...
/* discretize underneath a trace.  Insure that directly underneath the
 ...
 adjacent to those right underneath, no bigger than width of trace .
 should we do more?*/
void do_trace(double x0, double y0, double z0, double x1, double y1, double z1,
		double width, double factor, Nonuni_gp *gp) {
	double max_x, max_y;
	double tan_th;
	...
	walk_along_line(x0 + x_orth * half_width, y0 + y_orth * half_width, z0,
			x1 + x_orth * half_width, y1 + y_orth * half_width, z1, max_x,
			max_y, gp);
	walk_along_line(x0 - x_orth * half_width, y0 - y_orth * half_width, z0,
			x1 - x_orth * half_width, y1 - y_orth * half_width, z1, max_x,
			max_y, gp);
	...
	walk_along_line(x0 + 3 * x_orth * half_width, y0 + 3 * y_orth * half_width,
			z0, x1 + 3 * x_orth * half_width, y1 + 3 * y_orth * half_width, z1,
			max_x * 2, max_y * 2, gp);
	walk_along_line(x0 - 3 * x_orth * half_width, y0 - 3 * y_orth * half_width,
			z0, x1 - 3 * x_orth * half_width, y1 - 3 * y_orth * half_width, z1,
			3 * max_x, 3 * max_y, gp);
