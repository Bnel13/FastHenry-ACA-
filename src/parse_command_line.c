...
ind_opts *Parse_Command_Line(int argc, char **argv) {
...
#ifdef SU0914
	opt_list = gather_opts(argc, argv,
			"s:m:p:o:l:f:g:a:d:r:hMVvk:t:b:c:e:i:D:x:S:R:P:");
#else
	opt_list =
	...
	for (; opt_list != NULL && errflag == 0; opt_list = opt_list->next) {
		...
		case 'm':
			...
			else if (strncmp(opt_list->arg, "multi", 5) == 0)
		...
		case 'r':
			if (sscanf(opt_list->arg, "%d", &opts->orderROM) != 1) {
				fprintf(stderr,
						"Unspecified order for reduced order model: %s\n",
						opt_list->arg);
				errflag++;
			}
			break;
...
#ifdef SU0914
		case 'V':
			opts->show_version = ON;
			break;

		case 'P':
			opts->output_currents = ON;
			opts->cur_fname = opt_list->arg;
			declare_current_output_file(opts->cur_fname);
			break;
#endif // SU0914
        //Start Bap
		case 'z':
		...
		case 'c':
			if (sscanf(opt_list->arg, "%d", &opts->maxiters) != 1) {
				fprintf(stderr,
						"Can't interpret maximum number of iteration: %s\n",
						opt_list->arg);
				errflag++;
			}
			break;
		...
		case '\0':
			if (opts->fname == NULL) {
				...
				fprintf(stderr,
						"Two files specified: %s and %s. Specify only one.\n",
						opts->fname, opt_list->arg);
				errflag++;
			}
			break;
		...
		default:
			fprintf(stderr,
					"Unknown option %c which has slipped past gather_opts\n",
					opt_list->op);
			errflag++;
			break;
...
void fix_and_print_opts(ind_opts *opts) {
	...
	if (gethostname(hostname, BUFSIZ) != -1)
		...
		fprintf(stdout, "  Host: ? (gethostname() failure)\n");
#else
	...
	if (opts->makeFastCapFile != OFF) {
		...
		if (opts->mat_vect_prod == DIRECT)
			...
			printf("Matrix vector product method: MLACA-SVD\n");
