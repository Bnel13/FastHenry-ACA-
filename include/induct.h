...
#define FHVERSION "3.1wr+su64+ACA"
#else
#define FHVERSION "3.1wr+su32+ACA"
#endif // __LP64__
#define FHDATE "29Sep96, mod 082514 (wr), mod 11Sept14 (su)"
#else
...
#ifdef SU0914
	char* vfname; /* Filename of version info output file. */
	int show_version; /* Show version info, ON or OFF */
	int output_currents; /* Output port currents to file, ON or OFF */
	char* cur_fname; /* Filename of port current output file. */
#endif
} /* ind_opts */;
...
//int ACA1(int , int, charge**, charge**,double**,double**);//BAP change
//int ACA2(int, int, charge**, charge**);//BAP change
int ACA_new(int, int, int*, int*, charge**, double**, double**, double);//BAP change
int SVD_QR(double **, double **, int, int, int, double);//BAP change
double aanalitic(FILAMENT *, FILAMENT *, int, int, int);//bap change
double mutual_quadrature(FILAMENT *, FILAMENT *, int, int, int, int, int, int);//bap change
//void xyz_point(double arr[], double arr[], double arr[], double, double, double);
//double aanalitic2(FILAMENT *, FILAMENT *, int, int, int);//bap change
//double direct(charge *, charge *);//bap change
//void do_direct(ssystem *, int ,int);
void Set_up_MLACA(ssystem *, int);//BAP change
void top_level(ssystem *, int);//BAP change
void MLACA_cal(int, int *, int **, int ***, int **, int ***, charge **, ssystem *, double);//BAP change
#endif
